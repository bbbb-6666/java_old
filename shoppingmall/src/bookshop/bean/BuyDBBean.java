...
public class BuyDBBean { 
	//buyDBBean : 데이터베이스에 접속해서 연결하고 데이터를 처리해주는 역할
    ...
    public List<String> getAccount(){
        ...
        ResultSet rs = null; //sql에 대한 결과값
        List<String> accountList = null;
        try {
            conn = getConnection();
                        
            pstmt = conn.prepareStatement("select * from bank");
            rs = pstmt.executeQuery(); //bank 테이블의 모든 정보들을 rs에다가 넣음.
            ...
            while (rs.next()) {
           	  String account = new String(rs.getString("account")+" "
                     + rs.getString("bank")+" "+rs.getString("name")); 
           	  			//account, bank, name이라는 필드에서 정보를 가져옴.
